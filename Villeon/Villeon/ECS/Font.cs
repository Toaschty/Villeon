    ...
    public class Font
    {
        ...
        public Font(Color4 color, Texture2D fontTexture, string fontJsonPath)
        {
            ...
            for (int i = 0; i < charCount; i++)
            {
                ...
                {
                    new Vector2(x * cellWidth, (y * cellHeight) - charHeight) / (float)_sheetTexture.Height,                // bottomLeft
                    new Vector2((x * cellWidth) + charWidth, (y * cellHeight) - charHeight) / (float)_sheetTexture.Height,  // bottomRight
                    new Vector2(x * cellWidth, y * cellHeight) / (float)_sheetTexture.Height,                               // topLeft
                    new Vector2((x * cellWidth) + charWidth, y * cellHeight) / (float)_sheetTexture.Height,                 // topRight
                };
